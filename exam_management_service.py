...
class ExamManagementService:
    ...
    def get_student_aggregate_report(self, exam_ids: List[int], student_id: int) -> Dict:
        ...
        # 1. Get info
        self.cursor.execute("SELECT AdmNo, FName, SName as LName FROM studentinfo WHERE AdmNo = %s AND school_id = %s", (student_id, self.school_id))
        ...
        sql_marks = f"""
            ...
            FROM exam_marks em
            JOIN subjects s ON em.subject_id = s.subjectNo AND em.school_id = s.school_id
            WHERE em.student_id = %s AND em.school_id = %s AND em.exam_id IN ({format_strings})
        """
        self.cursor.execute(sql_marks, (student_id, self.school_id, *exam_ids))
    ...
    def get_all_exams(self, academic_year_id: Optional[int] = None) -> List[Dict]:
        ...
        sql = """
            SELECT e.*, ay.year as academic_year_name, 
                   (SELECT COUNT(*) FROM exam_marks WHERE exam_id = e.id AND school_id = %s) as marks_count
            FROM exam_series e
            JOIN academic_years ay ON e.academic_year_id = ay.id AND e.school_id = ay.school_id
            WHERE e.school_id = %s
        """
    ...
    def get_grade_for_mark(self, mark: float, scale_id: Optional[int] = None) -> Optional[Dict]:
        ...
        if scale_id:
            sql = "SELECT * FROM grading_details WHERE scale_id = %s AND %s BETWEEN min_mark AND max_mark AND school_id = %s"
            self.cursor.execute(sql, (scale_id, mark, self.school_id))
        else:
            # Use default scale
            sql = """
                SELECT gd.* 
                ...
                WHERE gs.is_default = TRUE AND %s BETWEEN gd.min_mark AND gd.max_mark AND gd.school_id = %s
            """
            self.cursor.execute(sql, (mark, self.school_id))
        ...
        try:
            ...
            if not is_absent and mark is not None:
                ...
                sql_class = """
                    ...
                    WHERE student_id = %s AND is_current = TRUE AND school_id = %s
                    LIMIT 1
                """
    ...
    def get_marks_for_class_subject(self, exam_id: int, class_id: int, subject_id: int) -> List[Dict]:
        ...
        sql = """
            ...
            WHERE ca.class_id = %s AND s.school_id = %s
    ...
    def get_student_results(self, student_id: str, exam_id: int) -> Dict:
        ...
        sql = """
            ...
            JOIN subjects sub ON ss.subject_id = sub.subjectNo AND ss.school_id = sub.school_id
            LEFT JOIN exam_marks m ON ca.student_id = m.student_id AND m.subject_id = sub.subjectNo AND m.exam_id = %s AND ca.school_id = m.school_id
            LEFT JOIN grading_details gd ON m.grade_id = gd.id AND m.school_id = gd.school_id
            WHERE ca.student_id = %s AND ca.is_current = TRUE AND ca.school_id = %s
        """
        self.cursor.execute(sql, (exam_id, student_id, self.school_id))
    ...
    def get_report_card_data(self, student_id: str, exam_id: int) -> Dict:
        ...
        sql_info = """
            ...
            JOIN class_allocation ca ON s.AdmNo = ca.student_id AND ca.is_current = TRUE AND s.school_id = ca.school_id
            JOIN classes c ON ca.class_id = c.classID AND ca.school_id = c.school_id
            JOIN exam_series e ON e.id = %s AND ca.school_id = e.school_id
            JOIN academic_years ay ON e.academic_year_id = ay.id AND e.school_id = ay.school_id
            WHERE s.AdmNo = %s AND s.school_id = %s
        """
        self.cursor.execute(sql_info, (exam_id, student_id, self.school_id))
    ...
    def get_class_tabulation(self, exam_id: int, class_id: int) -> Dict:
        ...
        """
        self.cursor.execute(sql_subs, (class_id, self.school_id))
        ...
        sql_students = """
            ...
            ORDER BY s.FName, s.SName
        """
        self.cursor.execute(sql_students, (class_id, self.school_id))
        ...
        sql_marks = """
            ...
            FROM exam_marks m
            JOIN grading_details gd ON m.grade_id = gd.id AND m.school_id = gd.school_id
            WHERE m.exam_id = %s AND m.school_id = %s AND m.student_id IN (
                SELECT student_id FROM class_allocation WHERE class_id = %s AND is_current = TRUE AND school_id = %s
            )
        """
        self.cursor.execute(sql_marks, (exam_id, self.school_id, class_id, self.school_id))
    ...
    def get_exam_rankings(self, exam_id: int, class_id: Optional[int] = None, limit: Optional[int] = None) -> List[Dict]:
        ...
        if class_id:
            ...
            self.cursor.execute("SELECT class_id FROM exam_classes WHERE exam_id = %s AND school_id = %s", (exam_id, self.school_id))
        ...
        for cid in target_class_ids:
            ...
            self.cursor.execute("SELECT display_name FROM classes WHERE classID = %s AND school_id = %s", (cid, self.school_id))
    ...
    def get_subject_winners(self, exam_id: int, class_id: Optional[int] = None) -> List[Dict]:
        ...
        if class_id:
            ...
            sql_subs = """
                SELECT DISTINCT s.subjectNo, s.subjName, s.code
                FROM exam_classes ec
                JOIN class_subjects cs ON ec.class_id = cs.class_id AND ec.school_id = cs.school_id
                JOIN subjects s ON cs.subject_id = s.subjectNo AND cs.school_id = s.school_id
                WHERE ec.exam_id = %s AND cs.is_active = TRUE AND ec.school_id = %s
            """
            self.cursor.execute(sql_subs, (exam_id, self.school_id))
        ...
        for sub in subjects:
            sql_winner = """
                ...
                FROM exam_marks em
                JOIN studentinfo si ON em.student_id = si.AdmNo AND em.school_id = si.school_id
                LEFT JOIN grading_details gd ON em.grade_id = gd.id AND em.school_id = gd.school_id
                JOIN class_allocation ca ON si.AdmNo = ca.student_id AND ca.is_current = TRUE AND si.school_id = ca.school_id
                JOIN classes c ON ca.class_id = c.classID AND ca.school_id = c.school_id
                WHERE em.exam_id = %s AND em.subject_id = %s AND em.school_id = %s
            """
            params = [exam_id, sub['subjectNo'], self.school_id]
            if class_id:
    ...
    def get_most_improved(self, exam_id: int, class_id: Optional[int] = None, limit: int = 5) -> List[Dict]:
        ...
        self.cursor.execute("SELECT academic_year_id, term, id FROM exam_series WHERE id = %s AND school_id = %s", (exam_id, self.school_id))
        ...
        sql_prev = """
            ...
            WHERE id < %s AND academic_year_id = %s AND school_id = %s
            ORDER BY id DESC LIMIT 1
        """
        self.cursor.execute(sql_prev, (exam_id, current['academic_year_id'], self.school_id))
        prev = self.cursor.fetchone()
        
        if not prev:
            # Try previous year if same year not found
            sql_prev_year = "SELECT id FROM exam_series WHERE academic_year_id < %s AND school_id = %s ORDER BY id DESC LIMIT 1"
            self.cursor.execute(sql_prev_year, (current['academic_year_id'], self.school_id))
            prev = self.cursor.fetchone()
        ...
        def get_all_totals(ex_id, c_id=None):
            sql = """
                ...
                FROM exam_marks em
                JOIN class_allocation ca ON em.student_id = ca.student_id AND ca.is_current = TRUE AND em.school_id = ca.school_id
                WHERE em.exam_id = %s AND em.school_id = %s
            """
            params = [ex_id, self.school_id]
            if c_id:
        ...
        for sid, cur in current_totals.items():
            if sid in prev_totals:
                ...
                if diff > 0:
                    # Get student name
                    self.cursor.execute("SELECT AdmNo, CONCAT(FName, ' ', SName) as name FROM studentinfo WHERE AdmNo = %s AND school_id = %s", (sid, self.school_id))
    ...
    def get_all_grading_scales(self) -> List[Dict]:
        ...
        self.cursor.execute("SELECT * FROM grading_scales WHERE school_id = %s ORDER BY is_default DESC, name ASC", (self.school_id,))
        return self.cursor.fetchall()
    ...
    def save_grading_details(self, scale_id: int, grades: List[Dict]) -> bool:
        ...
        try:
            ...
            sql = """
                INSERT INTO grading_details (scale_id, grade, min_mark, max_mark, points, remarks, class_teacher_remarks, principal_remarks, school_id)
    ...
    def assign_scale_to_class(self, class_id: int, scale_id: Optional[int]) -> bool:
        ...
        try:
            sql = "UPDATE classes SET grading_scale_id = %s WHERE classID = %s AND school_id = %s"
            self.cursor.execute(sql, (scale_id, class_id, self.school_id))
            self.connection.commit()
            return True
        except Exception as e:
            self.connection.rollback()
